...
Proof.
 unfold Qlt in |- *; abstract auto with *.
Defined.
...
Proof.
 ...
 apply pair_2; simpl in |- *; trivial; abstract omega.
 apply IHp; abstract omega.
 intro H_1_1.
 rewrite <- H_1_1.
 reflexivity.
Defined.
...
Proof.
 ...
 rewrite Zplus_comm.
 unfold Zsucc in |- *. 
 transitivity (n - 1 + 1)%Z; [ idtac | abstract omega ].
 apply f_equal2 with Z Z; trivial. 
 rewrite <- IHn.
 apply f_equal with Qpositive.
 apply f_equal3 with nat nat nat; trivial.
 simpl in |- *; abstract auto with arith. 
 abstract omega.
 intro H1; rewrite <- H1.
...
Proof.
 abstract (intros m; functional induction (Z_to_Q m); intros Hm;
...
Proof.
 abstract (intros [| q| q]; unfold Qlt in |- *; intros H;
...
Proof.
 abstract (intros x; induction  x as [x Hrecx| x Hrecx| ];
 ...
 intros p1 p2.
  ...
  case
   ...
   case
    ...
    repeat rewrite mult_1_l.
    repeat rewrite mult_1_r.
    clear H1 H2; apply Qpositive_c_equal_strong; trivial; try abstract omega;
     rewrite Hp2; rewrite nat_of_P_plus_morphism.
    abstract omega.
    replace (nat_of_P p1 + nat_of_P p - nat_of_P p) with (nat_of_P p1);
     [ apply lt_O_nat_of_P | abstract omega ].
    apply lt_O_nat_of_P.
    constructor.
    apply lt_O_nat_of_P.
    constructor. 
...
Proof.
 abstract (intros x y H; replace x with (Qplus x Zero); [ idtac | ring ];
...
Proof.
 abstract (intros a b Hab HabQ; apply Hab; apply eq_Z_to_Q; assumption).
Defined.
...
Proof.
 ...
 rewrite Z_to_Qplus; abstract (field; abstract discriminate).
Defined.
...
Proof.
 abstract (intros [| x| x] [| y| y] Hx Hy; trivial; simpl in |- *;
...
Proof.
 abstract (intros [| q| q] Hq; simpl in |- *; trivial; inversion Hq).
Defined.
...
Proof.
 abstract (intros a b q; repeat rewrite Zopp_eq_mult_neg_1;
...
Proof.
 intros [| px| px]; abstract solve [ left; trivial | right; discriminate ].
Defined.         
 
Lemma Qinv_resp_nonzero : forall x : Q, x <> Zero -> Qinv x <> Zero.
Proof.
 abstract (intros [| px| px] Hx; solve [ Falsum | discriminate ]).
Defined. 
...
Proof.
 intros [| px| px] Hx; abstract discriminate Hx || reflexivity.
Defined.
...
Proof.
 abstract (intros [| px| px] [| py| py] Hx Hy; trivial; solve
...
Proof.
 abstract (intros [| x| x] Hx; trivial; inversion Hx).
Defined.
...
Proof.
 abstract (intros [|x|x] Hx; try discriminate; apply False_ind; apply Hx; apply Qlt_zero_pos).
Qed.

Lemma Qsgn_32:forall x:Q, Qsgn x <> 1 -> Qle x  Zero.
Proof.
 abstract(intros [|x|x] Hx; auto; try apply Qle_reflexive; apply False_ind; apply Hx; reflexivity).
Qed.

Lemma Qsgn_33:forall x:Q, Qle Zero x -> Qsgn x <> (-1)%Z.
Proof.
 abstract (intros [|x|x] Hx; try discriminate; apply False_ind; apply Hx; auto). 
Qed.
